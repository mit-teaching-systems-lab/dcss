...
const INVITE_STATUS_MESSAGES = {
  '1': {
    received: 'This invitation is pending. You may Accept, Decline or leave it unchanged.',
...
export const makeInviteExplanation = (props) => {
  ...
  const theScenario = (
    <Fragment>
      the scenario <strong>{scenario.title}</strong>, as <strong>{persona.name}</strong>
    </Fragment>
  );
  ...
  const place = cohort
    ? theScenarioInACohort
    : theScenario;

  return isRecipient ? (
    <Fragment>
      <strong><Username user={user} /></strong>{' '}
      invited you to join{' '}{place}
    </Fragment>
  ) : (
    <Fragment>
      You invited <strong><Username user={user} /></strong>{' '}
      to{' '}{place}
    </Fragment>
  );
};
...
class UserInvitesList extends Component {
  ...
  render() {
    ...
    return (
      <Invite.Group>
        {invites.length ? invites.reduce((accum, invite) => {

          if (!invite.persona_id ||
              !invite.receiver_id ||
              !invite.scenario_id ||
              !invite.sender_id) {
            return accum;
          ...
          const orButton = <Button.Or key={Identity.key({ key, or: true })} />;

          const buttons = isRecipient
            ? [acceptButton, orButton, declineButton]
            : [cancelButton]

          const shouldShowButtons = invite.status === INVITE_STATUS_PENDING_NAME;
              ...
              <Invite.Content>
                ...
                <Invite.Actions>
                  {shouldShowButtons ? (
                    ...
                    <Text>
                      {INVITE_STATUS_MESSAGES[INVITE_STATUS_MAP[invite.status]][type]}{' '}
...
const mapStateToProps = (state, ownProps) => {
  const { cohortsById, invites, scenarios, scenariosById, user, usersById } = state;
  const invitesByCode = invites.reduce((accum, invite) => ({
    ...accum,
    [invite.code]: invite
  }), {});

  return { invites, invitesByCode, cohortsById, scenarios, scenariosById, user, usersById };
};
