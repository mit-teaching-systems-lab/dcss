...
test('Render 6 1', async done => {
  ...
  const sComponent = shallow(<ConnectedRoutedComponent {...props} />);
  sComponent.setProps({ required: false });
  expect(snapshotter(sComponent)).toMatchSnapshot();

  const fComponent = mount(<ConnectedRoutedComponent {...props} />);
  fComponent.setProps({ required: false });
  expect(snapshotter(fComponent)).toMatchSnapshot();
